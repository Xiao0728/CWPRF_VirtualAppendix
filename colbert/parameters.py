import torch

DEVICE = torch.device("cuda")

SAVED_CHECKPOINTS = [200, 400, 600, 800, 500]
SAVED_CHECKPOINTS = [32*1000, 100*1000, 150*1000, 180*1000,200*1000,250*1000,280*1000, 300*1000, 330*1000, 350*1000,380*1000, 400*1000,430*1000]
SAVED_CHECKPOINTS += [10*1000, 20*1000, 30*1000, 40*1000, 50*1000, 60*1000, 70*1000, 80*1000, 90*1000]
SAVED_CHECKPOINTS += [25*1000, 50*1000, 75*1000]
SAVED_CHECKPOINTS += [1000,2*1000,3*1000,4*1000,5*1000,6*1000,7*1000,8*1000,9*1000]
SAVED_CHECKPOINTS += [11*1000,12*1000,13*1000,14*1000,15*1000,16*1000,17*1000,18*1000,19*1000,44*1000]
SAVED_CHECKPOINTS = set(SAVED_CHECKPOINTS)
# import torch

# DEVICE = torch.device("cuda")

# # SAVED_CHECKPOINTS = [32*1000, 100*1000, 150*1000, 180*1000,200*1000,250*1000,280*1000, 300*1000, 330*1000, 350*1000,380*1000, 400*1000,430*1000]
# # SAVED_CHECKPOINTS += [450*1000,480*1000, 500*1000,550*1000,600*1000,650*1000, 700*1000, 750*1000, 800*1000]
# # SAVED_CHECKPOINTS += [10*1000, 20*1000, 30*1000, 40*1000, 44*1000, 50*1000, 60*1000, 70*1000, 80*1000, 90*1000]
# # SAVED_CHECKPOINTS += [25*1000, 50*1000, 75*1000]

# SAVED_CHECKPOINTS = set(SAVED_CHECKPOINTS)
