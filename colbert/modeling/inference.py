...
from colbert.modeling.tokenization import QueryTokenizer, DocTokenizer,PRFQueryTokenizer
...
class ModelInference():
    def __init__(self, colbert: ColBERT, amp=False):
        ...
        self.prf_query_tokenizer = PRFQueryTokenizer(colbert.query_maxlen)
    ...
    def prfquery(self, *args, to_cpu=False, **kw_args):
        with torch.no_grad():
            with self.amp_manager.context():
                Q = self.colbert.query(*args, **kw_args)
                return Q.cpu() if to_cpu else Q
    ...
    def prfqueryFromText(self, prfqueries, bsize=None, to_cpu=False, with_ids = False):
        """
        the input: prfqueries should be the format: 
        """
        if bsize:
            batches = self.prf_query_tokenizer.tensorize(prfqueries, bsize=bsize)
            batchesEmbs = [self.prfquery(input_ids, attention_mask, to_cpu=to_cpu) for input_ids, attention_mask in batches]
            if with_ids:
                return torch.cat(batchesEmbs), torch.cat([ids for ids, _ in batches]), torch.cat([masks for _, masks in batches])
            return torch.cat(batchesEmbs)
        
        input_ids, attention_mask = self.prf_query_tokenizer.tensorize(prfqueries)
        if with_ids:
            return (self.prfquery(input_ids, attention_mask), input_ids, attention_mask)
        return self.prfquery(input_ids, attention_mask)
